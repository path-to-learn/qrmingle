...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  adminRoutes.get('/global-analytics', async (req, res) => {
    try {
      ...
      // If there's no actual country data (which seems to be the case currently),
      // provide some test data for demonstration purposes
      if (countryData.length === 0) {
        // Sample country data for demonstration
        const testCountryData = [
          { country: 'United States', countryCode: 'US', visitors: 42 },
          { country: 'India', countryCode: 'IN', visitors: 38 },
          { country: 'United Kingdom', countryCode: 'GB', visitors: 25 },
          { country: 'Germany', countryCode: 'DE', visitors: 18 },
          { country: 'Canada', countryCode: 'CA', visitors: 15 },
          { country: 'Australia', countryCode: 'AU', visitors: 12 },
          { country: 'France', countryCode: 'FR', visitors: 10 },
          { country: 'Japan', countryCode: 'JP', visitors: 8 },
          { country: 'Brazil', countryCode: 'BR', visitors: 7 },
          { country: 'China', countryCode: 'CN', visitors: 5 }
        ];
